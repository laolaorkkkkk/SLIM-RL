...
def get_gdpo_config(n_points):
    # return nodes, weights in [-1, 1]
    ...
    assert abs(np.sum(weights) - 1.0) < 1e-6, "Quadrature weights must sum to 1"
    
    return t_nodes, weights
def generate_monotonic_pmasks_df(t_node: float, num_blocks: int, spread: float = 0.4, shape: str = "linear", device=None):
    if num_blocks == 1:
        return torch.tensor([t_node], device=device)
    
    max_symmetric_spread = 2.0 * min(t_node, 1.0 - t_node)
    actual_spread = min(spread, max_symmetric_spread - 1e-6)
    actual_spread = max(0.0, actual_spread)
    
    if shape == "cosine":
        x = torch.linspace(0, 1, num_blocks, device=device)
        curve = 0.5 * (1 - torch.cos(np.pi * x))
        curve = curve - 0.5 # range approx [-0.5, 0.5]
        p_masks = t_node + actual_spread * curve
    else: # linear 
        p_start = t_node - actual_spread / 2
        p_end = t_node + actual_spread / 2
        p_masks = torch.linspace(p_start, p_end, num_blocks, device=device)
    
    return p_masks


def generate_monotonic_pmasks_df_random(t_node: float, num_blocks: int, spread: float = 0.3, device=None):

    if num_blocks == 1:
        return torch.tensor([t_node], device=device)
    
    p_start = max(0.05, t_node - spread / 2 + torch.rand(1, device=device).item() * 0.1 - 0.05)
    
    increments = torch.rand(num_blocks - 1, device=device)
    increments = increments / increments.sum() * spread
    
    p_masks = torch.zeros(num_blocks, device=device)
    p_masks[0] = p_start
    p_masks[1:] = p_start + torch.cumsum(increments, dim=0)
    
    p_masks = torch.clamp(p_masks, 0.0, 1.0)
