...
if __name__ == "__main__":
    ...
    # --- TPF (tokens per forward) — computed from step_map BEFORE it is optionally cleared
    #     below. step_map[k] = denoising step (>=1) at which token k was committed; the 0
    #     sentinel (never-unmasked / force-committed) is excluded so the count reflects real
    #     forward passes. TPF = committed_tokens / distinct_forward_passes (per response, averaged).
    tpf_list, fwd_list = [], []
    for rec in data:
        for sm in rec.get("step_map", []):
            if not sm or not isinstance(sm, (list, tuple)):
                continue
            steps = [s for s in sm if isinstance(s, (int, float)) and s >= 1]  # drop 0-sentinel
            if steps:
                n_fwd = len(set(steps))
                tpf_list.append(len(steps) / n_fwd)
                fwd_list.append(n_fwd)
    avg_tpf = sum(tpf_list) / len(tpf_list) if tpf_list else 0.0
    avg_fwd = sum(fwd_list) / len(fwd_list) if fwd_list else 0.0
