...
if __name__ == "__main__":
    ...
    for i in range(num_slice):
        # Mirror the rollout's naming (sample/sdar_rl_rollout.py): with >1 node each shard is
        # outputs-{idx}-..., but for a single node the rollout writes outputs-... (no index).
        if num_slice > 1:
            file_name_sub = "../" + project_name + f"/temp_data/outputs-{i}-" + outputs_name + ".json"
        else:
            file_name_sub = "../" + project_name + f"/temp_data/outputs-" + outputs_name + ".json"
