...
def _remote_cmd(host: str, cmd: str) -> str:
    # DLLM_REMOTE_LAUNCHER lets a SLURM job override the default `ssh root@<host>`
    # dispatch (which does NOT work on many SLURM clusters: no root login between compute nodes).
    # Set it to e.g. "srun --overlap --nodes=1 --ntasks=1 --nodelist={host}" and we
    # run the command on the target node via srun inside the existing allocation,
    # avoiding inter-node ssh/auth entirely. {host} is substituted with the node name.
    launcher = os.environ.get("DLLM_REMOTE_LAUNCHER")
    if launcher:
        prefix = launcher.replace("{host}", host)
        return f'{prefix} bash -lc {shlex.quote(cmd)}'
    return f'ssh root@{host} "bash -lc {shlex.quote(cmd)}"'

def run_remote(host: str, cmd: str, check: bool = True) -> None:
    subprocess.run(_remote_cmd(host, cmd), shell=True, check=check)

def run_remote_async(host: str, cmd: str) -> subprocess.Popen:
    return subprocess.Popen(_remote_cmd(host, cmd), shell=True)
...
if __name__ == "__main__":
    ...
    def env_prefix() -> str:
        # DLLM_ENV_PREFIX lets a SLURM job replace the default conda activation
        # (`source ~/.bashrc && source activate <env_name>`) with a custom
        # module+venv flow, e.g.:
        #   "module load <your-toolchain> && source /path/to/venv/bin/activate && "
        # The value is used verbatim as the command prefix on EVERY node (local + remote).
        override = os.environ.get("DLLM_ENV_PREFIX")
        if override:
            return override
        return (
            "source ~/.bashrc && "
    ...
    def sample(worker_hosts, epoch, cfg, type, model_base, block_size = None, top_k = None, remasking_strategy = None):
        ...
        elif model_base in ("slim_rl_sdar", "slim_rl_sdar_align"):
            script_name = "sdar_rl_rollout.py"
    ...
    def train(worker_hosts, epoch, cfg, model_base, target = None):
        ...
        if target is None:
            ...
            elif model_base == "slim_rl_sdar":
                script_name = "slim_rl_sdar.py"
            elif model_base == "slim_rl_sdar_align":
                script_name = "slim_rl_sdar_align.py"
            elif model_base == "trado":
    ...
    if OmegaConf.select(cfg, "model.value_base_model") is not None:
    ...
    # On SLURM we skip init_hosts: it root-ssh's into each worker to append
    # exports + `conda config` to ~/.bashrc, which neither applies nor works here
    # (no root ssh, and we use module+venv via DLLM_ENV_PREFIX instead of conda).
    if not os.environ.get("DLLM_SKIP_INIT_HOSTS"):
    ...
    while epoch <= total_step:
        ...
        if epoch % cfg.experiment.eval_every == 0:
            if model_base in ["sdar", "trado", "slim_rl_sdar", "slim_rl_sdar_align"]:
                ...
                remasking_strategy_list = cfg.evaluation.remasking_strategy
                if OmegaConf.select(cfg, "evaluation.top_k") is not None:
