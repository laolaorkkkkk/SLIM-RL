...
# 2) mark special tokens (can be toggled hidden in the frontend)
try:
...
# 3) write out a self-contained HTML file
import json, os
...
# per-token incremental pieces, kept consistent with tokenizer decoding
pieces = []
prev = ""
for i in range(len(token_ids)):
    cur = tokenizer.decode(token_ids[:i+1], skip_special_tokens=False, clean_up_tokenization_spaces=False)
...
# mark special tokens (for optional hiding)
try:
    special_ids = set(getattr(tokenizer, "all_special_ids", []) or [])
except Exception:
    special_ids = set()
