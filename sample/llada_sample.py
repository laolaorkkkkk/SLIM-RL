...
def generate_with_prefix_cache(
        ...
        remasking_strategy=None, tau_budget_m=1.0
    ) -> DiffusionOutput:
    ...
    for blk in range(num_blocks):
        ...
        x0, tr_idx = get_transfer_index(
            ...
            x, num_transfer[:, 0], unmask_threshold, remasking_strategy=remasking_strategy, tau_budget_m=tau_budget_m)
        x[tr_idx] = x0[tr_idx]
            ...
            if use_cache:
                if cgws is not None:
                    ...
                    x0, tr_idx = get_transfer_index(
                        ...
                        mask_blk, x[:, window_slice], num_transfer[:, i], unmask_threshold, remasking_strategy=remasking_strategy, tau_budget_m=tau_budget_m)
                    x[:, window_slice][tr_idx] = x0[tr_idx]
                else:
                    logits = model(x[:, s:], past_key_values=pkv, use_cache=True).logits
                    x0, tr_idx = get_transfer_index(
                        logits, temperature, target,
                        mask_blk, x[:, s:], num_transfer[:, i], unmask_threshold, remasking_strategy=remasking_strategy, tau_budget_m=tau_budget_m)
...
def get_transfer_index(logits, temperature, target, mask_index, x, num_transfer_tokens, threshold=None,
    ...
    if remasking_strategy == "tau_budget_dynamic" and threshold is not None:
        # τ-budget dynamic unmasking — faithful port of the SDAR jetengine scheduler
        # (jetengine_ext/engine/scheduler.py tau_budget_dynamic branch): per row, among
        # tokens with confidence > τ (=threshold), admit highest-confidence tokens until
        # cumulative uncertainty (1-conf) exceeds budget = m·(1-τ); always unmask at least
        # the static-schedule count num_transfer_tokens (progress floor → block still finishes).
        tau = threshold
        confidence = x0_p.masked_fill(~mask_index, float("-inf"))   # (B, T)
        transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
        for j in range(confidence.shape[0]):
            n_masked = int(mask_index[j].sum().item())
            if n_masked == 0:
                continue
            base_k = int(num_transfer_tokens[j].item() if torch.is_tensor(num_transfer_tokens[j]) else num_transfer_tokens[j])
            m_eff = tau_budget_m if tau_budget_m > 0 else base_k   # jetengine: m falls back to step base count
            budget = m_eff * (1.0 - tau)
            conf_j = confidence[j]
            above_conf = conf_j[conf_j > tau]
            if above_conf.numel() > 0:
                sorted_conf, _ = torch.sort(above_conf, descending=True)
                cumsum_u = torch.cumsum(1.0 - sorted_conf, dim=0)
                k_budget = int((cumsum_u <= budget).sum().item())
            else:
                k_budget = 0
            k = min(max(k_budget, base_k), n_masked)
            if k <= 0:
                continue
            _, sel = torch.topk(conf_j, k)
...
def worker(pretrained_model, rank, prompts, orig_idx, seq_dict, step_dict, batch_size, config):
    ...
    for start in tqdm(range(0, len(prompts), batch_size),
        ...
        # τ-budget is a "dynamic" strategy (it needs a threshold τ); the algorithm itself
        # is selected by remasking_strategy inside get_transfer_index. m defaults to 1.
        tau_budget_m = float(config.rollout.get("tau_budget_m", 1.0))
        ...
        out = generate_with_prefix_cache(
            ...
            remasking_strategy=config.rollout.remasking_strategy, tau_budget_m=tau_budget_m
        )
