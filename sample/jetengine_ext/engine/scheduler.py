        ...
        if run_type == RunType.PREFILL:
            ...
            for seq in seqs:
                ...
                if seq.status == SequenceStatus.DENOISING:
                    ...
                    elif 'contiguous_dynamic' in seq.remasking_strategy:
                        confidence = torch.where(mask_index, seq_x0_p, -np.inf)
                        selected = (confidence > seq.dynamic_threshold) & mask_index

                        first_masked_pos = mask_index.nonzero(as_tuple=True)[0].min().item() if mask_index.any() else -1
                        filtered = torch.zeros_like(selected)

                        sel_cpu = selected.tolist()
                        n = len(sel_cpu)
                        i = 0
                        while i < n:
                            if not sel_cpu[i]:
                                ...
                                continue
                            run_start = i
                            while i < n and sel_cpu[i]:
                                i += 1
                            run_len = i - run_start
                            if run_len >= 2:
                                filtered[run_start:i] = True
                            elif run_start == first_masked_pos:
                                filtered[run_start] = True

                        if filtered.sum().item() < num_to_transfer:
                            k = min(num_to_transfer, mask_index.sum().item())
                            _, top_indices = torch.topk(confidence, k)
                            filtered[top_indices] = True
                        transfer_index = filtered
                        num_to_transfer = transfer_index.sum().item()

                    elif 'nlc_bounded_dynamic' in seq.remasking_strategy:
                        confidence = torch.where(mask_index, seq_x0_p, torch.tensor(0.0, device=seq_x0_p.device))
                        nlc = torch.where(
                            mask_index,
                            -torch.log(confidence.clamp(min=1e-8)),
                            torch.tensor(float('inf'), device=seq_x0_p.device),
                        )
                        nlc_sorted, order = torch.sort(nlc)
                        cumsum_nlc = torch.cumsum(nlc_sorted, dim=0)
                        num_masked = mask_index.sum().item()
                        valid_count = (cumsum_nlc[:num_masked] <= seq.nlc_budget).sum().item()
                        k = max(valid_count, num_to_transfer)
                        k = min(k, num_masked)
                        if k > 0:
                            selected = order[:k]
                    ...
                    elif 'gc_dynamic' in seq.remasking_strategy:
                        confidence = torch.where(mask_index, seq_x0_p, -np.inf)
                        remaining = mask_index.sum().item()
                        remaining_steps = max(1, seq.denoising_steps - seq.current_denoising_step)

                        min_pace = math.ceil(remaining / remaining_steps)
                        half_cap = max(1, remaining // 2)
                        cap = max(min_pace, half_cap)

                        above_threshold = (confidence > seq.dynamic_threshold)
                        n_above = above_threshold.sum().item()

                        k = max(min_pace, min(n_above, cap))
                        k = min(k, remaining)
                    ...
                    elif 'tau_budget_dynamic' in seq.remasking_strategy:
                        confidence = torch.where(mask_index, seq_x0_p, -np.inf)
                        above = confidence > seq.dynamic_threshold
                        above_conf = confidence[above]
                        if above_conf.numel() > 0:
                            sorted_conf, _order = torch.sort(above_conf, descending=True)
                            m = seq.tau_budget_m if seq.tau_budget_m > 0 else num_to_transfer
                            budget = m * (1.0 - seq.dynamic_threshold)
                            uncertainties = 1.0 - sorted_conf
                            cumsum_u = torch.cumsum(uncertainties, dim=0)
                            within_budget = (cumsum_u <= budget)
                            k_budget = within_budget.sum().item()
                        else:
                            k_budget = 0
                        k = max(k_budget, num_to_transfer)
                        k = min(k, mask_index.sum().item())
                        _, top_indices = torch.topk(confidence, k)
                        transfer_index[top_indices] = True
                        num_to_transfer = k
                    ...
                    committed_conf = seq_x0_p[transfer_index]
                    seq.commit_counts.append(int(transfer_index.sum().item()))
                    seq.expected_wrong_commits.append(float((1.0 - committed_conf).sum().item()))
