    ...
    def step(self):
        ...
        finished_outputs = [
            (seq.seq_id, seq.completion_token_ids, seq.first_unmask_steps,
             seq.commit_counts, seq.expected_wrong_commits)
    ...
    def generate(
        ...
        with prof_ctx as prof:
            while not self.is_finished():
                ...
                for seq_id, token_ids, unmask_times, commit_counts, expected_wrong_commits in output:
                    outputs[seq_id] = {
                        "token_ids": token_ids,
                        "unmask_times": unmask_times,
                        "commit_counts": commit_counts,
                        "expected_wrong_commits": expected_wrong_commits,
                    }
                    if use_tqdm:
                        pbar.update(1)
        ...
        outputs = [
            {
                ...
                "commit_counts": item["commit_counts"],
                "expected_wrong_commits": item["expected_wrong_commits"],
            }
            for item in outputs
        ]
