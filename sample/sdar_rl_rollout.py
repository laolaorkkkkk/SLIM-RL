...
# The two rollout workers (one per GPU) compile identical kernels and race to
# os.replace into the SHARED triton dir above. On a warm inductor FxGraph-cache HIT,
# TritonBundler.read_and_emit restores the bundled kernel into that shared dir and the
# second worker hits OSError [Errno 39] Directory not empty. Disabling triton-bundling
# into the FxGraph cache removes that racy restore path entirely (FxGraph + triton
# caching themselves stay enabled for speed).
_os.environ["TORCHINDUCTOR_BUNDLE_TRITON_INTO_FX_GRAPH_CACHE"] = "0"
...
if __name__ == "__main__":
    ...
    sampling_kwargs = dict(
        ...
        nlc_budget           = config.rollout.get("nlc_budget", 5.0),
        tau_budget_m         = config.rollout.get("tau_budget_m", 0),
    )
    ...
    if ngroups == 1:
        ...
        store_ports = []
        while len(store_ports) < ngroups:
            port = _find_free_port()
            if port not in store_ports:
                store_ports.append(port)
    ...
    for item in seq_pairs:
        if len(item) == 2:
            ...
            gi, text, steps, cc, ewc = item
        restored_outputs[gi] = text
        restored_steps[gi]   = steps
        restored_commit_counts[gi] = cc
        restored_ewc[gi] = ewc
    ...
    for i in range(N):
        ...
        if restored_steps[i] is None:
            restored_steps[i] = ""
        if restored_commit_counts[i] is None:
            restored_commit_counts[i] = []
        if restored_ewc[i] is None:
            restored_ewc[i] = []
    ...
    for full_output in restored_outputs:
        ...
        data[index_i]["commit_counts"].append(restored_commit_counts[i] if restored_commit_counts[i] else [])
        data[index_i]["expected_wrong_commits"].append(restored_ewc[i] if restored_ewc[i] else [])
