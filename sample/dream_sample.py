...
def _sample(
    ...
    for num_block in range(num_blocks):
        ...
        while True:
            ...
            if remasking_strategy == "tau_budget_dynamic" and unmask_threshold is not None:
                # τ-budget dynamic unmasking — port of jetengine scheduler.py tau_budget_dynamic
                # (budget = m*(1-τ); floor = the static-schedule per-step count). full_target has
                # -inf at non-mask positions. ASSUMES target in {confidence, margin_confidence}
                # (scores in [0,1]); neg_entropy is unbounded and not calibrated for (1-conf).
                if cgws is not None:
                    xwin = x[:, window_slice]
                else:
                    xwin = x[:, current_block_start:]
                tau = unmask_threshold
                t = timesteps[num_block][i]
                # Static-schedule floor is computed PER ROW (jetengine's num_to_transfer is a
                # per-sequence count, scheduler.py:190-208). At the final scheduled step s->0 so the
                # floor == n_masked => every row flushes itself and the block completes within spb
                # steps. (The earlier port used the BATCH AVERAGE int(num_mask_token) as the floor;
                # under tau-budget rows unmask different counts and diverge, so a slow row stayed
                # masked at the last step, the while-loop ran one more time, and timesteps[i+1]
                # indexed past the size-(spb+1) schedule -> IndexError.)
                is_last = (i >= spb - 1) or (i + 1 >= timesteps[num_block].shape[0])
                frac = 1.0 if is_last else float(1.0 - timesteps[num_block][i + 1] / t)
                selected_map = torch.zeros_like(xwin, dtype=torch.bool)
                for j in range(full_target.shape[0]):
                    n_masked = int(mask_index[j].sum().item())
                    if n_masked == 0:
                        continue
                    conf_j = full_target[j]                 # -inf at non-mask
                    base_k = n_masked if is_last else int(n_masked * frac)
                    if is_last:
                        k = n_masked                        # flush all remaining masked tokens this row
                    else:
                        m_eff = tau_budget_m if tau_budget_m > 0 else base_k
                        budget = m_eff * (1.0 - tau)
                        above = conf_j[conf_j > tau]
                        if above.numel() > 0:
                            sc, _ = torch.sort(above, descending=True)
                            k_budget = int((torch.cumsum(1.0 - sc, dim=0) <= budget).sum().item())
                        else:
                            k_budget = 0
